...
How do we get that line, though, that that quantifies this relationship? OLS helps us pick the best line and determine the relationship by defining 'best' as a line that minimizes the average squared distance of the points in this stochastic process. However, note that the previous plot doesn't show us what origin (if any) to pass the line through ($\hat\beta_0$). This is the topic of today's formulation. The oft ignored and slightly maligned estimate for $\hat\beta_0$ does actually yield some interesting information, which is what would our dependent variable be if our explanatory variable was set to 0? In other words, what kind of utopia would we live in where Ferbie sales were noexistent and violent crime is but a memory? *Imagine*
...
Next post I will tackle the more difficult problem of using this newfound knowledge to derive the formula for $\hat\beta_1$